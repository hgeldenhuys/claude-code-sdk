...
async function testCompleteSystem() {
  ...
  try {
    ...
    console.log(`  • Dimensions updated: ${updateResult.dimensionsUpdated.join(', ')}`);
    ...
    console.log(`  • Entities: ${initialStats.entities} → ${updatedStats.entities} (+${updatedStats.entities - initialStats.entities})`);
    console.log(`  • Compositions: ${initialStats.compositions} → ${updatedStats.compositions} (+${updatedStats.compositions - initialStats.compositions})`);
    console.log(`  • Knowledge items: ${initialStats.knowledgeItems} → ${updatedStats.knowledgeItems} (+${updatedStats.knowledgeItems - initialStats.knowledgeItems})`);
    console.log(`  • Experiences: ${initialStats.experiences} → ${updatedStats.experiences} (+${updatedStats.experiences - initialStats.experiences})`);
    console.log(`  • Pain points: ${initialStats.painPoints} → ${updatedStats.painPoints} (+${updatedStats.painPoints - initialStats.painPoints})`);
    ...
    if (sseKnowledge) {
      ...
      console.log(`  • Confidence history entries: ${sseKnowledge.confidenceHistory.length}`);
    }
    ...
    console.log(`  • Ontology: ${contactsKnowledge.ontology ? '✓ Found' : '✗ Not found'}`);
    console.log(`  • Mereology: ${contactsKnowledge.mereology ? '✓ Found' : '✗ Not found'}`);
    console.log(`  • Epistemology: ${contactsKnowledge.epistemology ? '✓ Found' : '✗ Not found'}`);
    console.log(`  • Qualia: ${contactsKnowledge.qualia ? '✓ Found' : '✗ Not found'}`);
    ...
    console.log(`  • Epistemology: ${ssePattern.epistemology ? '✓ Found' : '✗ Not found'}`);
    ...
    console.log(`  • Ontology coverage: ${(awareness.health.ontologyCoverage * 100).toFixed(1)}%`);
    console.log(`  • Epistemic confidence: ${(awareness.health.epistemicConfidence * 100).toFixed(1)}%`);
    console.log(`  • Qualia depth: ${(awareness.health.qualiaDepth * 100).toFixed(1)}%`);

    console.log('\nConfidence distribution:');
    console.log(`  • Average: ${(awareness.confidence.average * 100).toFixed(1)}%`);
    console.log(`  • High confidence: ${awareness.confidence.high} concepts`);
    console.log(`  • Medium confidence: ${awareness.confidence.medium} concepts`);
    ...
    if (awareness.health.recommendations && awareness.health.recommendations.length > 0) {
      console.log('\nRecommendations:');
      awareness.health.recommendations.forEach(rec => console.log(`  • ${rec}`));
    }
    ...
    console.log(`\nKnowledge Growth:`);
    console.log(`  • Started with: ${initialStats.entities} entities, ${initialStats.painPoints} pain points`);
    console.log(`  • Learned: +${updatedStats.entities - initialStats.entities} entities, +${updatedStats.painPoints - initialStats.painPoints} pain points`);
